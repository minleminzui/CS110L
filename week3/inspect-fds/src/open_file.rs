...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let destination_path = fs::read_link(path).ok()?;
        let human_friendly = OpenFile::path_to_name(destination_path.to_str()?);

        let fdinfo = format!("/proc/{}/fdinfo/{}", pid, fd);
        let content = fs::read_to_string(fdinfo).ok()?;
        Some(OpenFile{
            name: human_friendly.to_string(),
            cursor: OpenFile::parse_cursor(&content).unwrap(),
            access_mode: OpenFile::parse_access_mode(&content).unwrap(),
        })
    }
